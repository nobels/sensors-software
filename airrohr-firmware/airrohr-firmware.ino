...
/*****************************************************************/
// increment on change
#define SOFTWARE_VERSION "NRZ-2017-100-Dustcube"
...
void webserver_values() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    String pm10text = F("<tr><td colspan='3'>Europese norm PM 10 per 24 uur is gemiddeld <b>50</b> µg/m3<br/>Europese norm PM10 is gemiddeld <b>40</b> µg/m3 per jaar<br/>WHO* advieswaarde voor PM 10 is <b>20</b> µg/m3 per jaar</td></tr>");
    String pm25text = F("<tr><td colspan='3'>Europese norm PM 2.5 is gemiddeld <b>25</b> µg/m3 per jaar<br/>Vanaf 2020 is de norm <b>20</b> µg/m3 per jaar<br/>WHO* advieswaarde voor PM 2.5 is <b>10</b> µg/m3 per jaar</td></tr>");
    last_page_load = millis();
    ...
    if (ppd_read) {
      ...
      page_content += table_row_from_value("PPD42NS", "PM 1",  last_value_PPD_P1, FPSTR(INTL_PARTIKEL_LITER));
      page_content += table_row_from_value("PPD42NS", "PM 2.5", last_value_PPD_P2, FPSTR(INTL_PARTIKEL_LITER));
      page_content += pm25text;
    }
    if (sds_read) {
      page_content += empty_row;
      page_content += table_row_from_value("SDS011", "PM 2.5", last_value_SDS_P2, "µg/m³");
      page_content += pm25text;
      page_content += table_row_from_value("SDS011", "PM 10", last_value_SDS_P1, "µg/m³");
      page_content += pm10text;      
    }
    if (pms24_read || pms32_read) {
      page_content += empty_row;
      page_content += table_row_from_value("PMS", "PM  1", last_value_PMS_P0, "µg/m³");
      page_content += table_row_from_value("PMS", "PM 2.5", last_value_PMS_P2, "µg/m³");
      page_content += pm25text;
      page_content += table_row_from_value("PMS", "PM 10", last_value_PMS_P1, "µg/m³");
      page_content += pm10text;      
    }
    if (dht_read) {
      page_content += empty_row;
    ...
    page_content += empty_row;
    page_content += table_row_from_value("Toestel", "Aantal Meetingen",  String(looper), "");
    page_content += table_row_from_value("Toestel", "IP-adres",  IPAddress2String(WiFi.localIP()), "");
    
    page_content += F("</table>");

    page_content += F("<br/>*WHO - World Health Organisation");
...
void display_values(const String& value_DHT_T, const String& value_DHT_H, const String& value_HTU21D_T, const String& value_HTU21D_H, const String& value_BMP_T, const String& value_BMP_P, const String& value_BMP280_T, const String& value_BMP280_P, const String& value_BME280_T, const String& value_BME280_H, const String& value_BME280_P, const String& value_PPD_P1, const String& value_PPD_P2, const String& value_SDS_P1, const String& value_SDS_P2) {
  ...
  if (has_display) {
    ...
    if (sds_read) {
      display.drawString(0, 10 * (value_count++), "PM 10: " + value_SDS_P1 + " µg/m³");
      display.drawString(0, 10 * (value_count++), "PM 2.5: " + value_SDS_P2 + " µg/m³");
    }
    ...
    //increment value
    display.drawString(0, 10 * (value_count++), "Metingen: " + String(looper));
